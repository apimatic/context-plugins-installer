...
const { tmpDir, cleanupAll, silenceConsole } = require('./helpers');

test.after(cleanupAll);

const REPO = 'apimatic/context-plugins';
const CTX = { plugin: 'xero-sdk', marketplace: 'context-plugins', repo: REPO };
...
const listing = (entries) => ({ code: 0, stdout: JSON.stringify(entries) });

const opts = (run) => ({ env: withClaude(), run });
