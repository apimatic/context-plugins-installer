...
test('a source that is also a destination is refused, not installed from', () => {
  // `replaceDir` removes its destination before copying, so this is the case that
  // would delete the user's plugin before reading it.
...
test('a source holding the generated marketplace is refused too', () => {
  ...
  // The generated marketplace lives under `root`, so a plugin rooted there would be
  // copied into itself.
...
test('a differently cased spelling of a destination is still a destination', () => {
  // Windows and macOS read these as one directory, and cmd.exe hands back the
  // lower-case drive letter by itself after `cd /d`.
  const root = tmpDir('cp-case-');
  const opts: PathOpts = { env: { CP_STATE_DIR: path.join(root, 'state') }, home: root };
  const dest = paths.cursorLocalDir(opts).join('my-sdk');
  fs.mkdirSync(path.join(dest.toString(), '.claude-plugin'), { recursive: true });
  fs.writeFileSync(
    path.join(dest.toString(), '.claude-plugin', 'plugin.json'),
