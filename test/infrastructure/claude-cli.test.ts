...
import { runnerFor, silenceConsole } from '../helpers.js';
...
/** For the lookup-only assertion: nothing should reach a spawn. */
const neverRuns = async (): Promise<RunResult> => {
...
test('findClaude looks for claude on the PATH it is given, not the host one', () => {
  assert.equal(findClaude(runnerFor(neverRuns, { PATH: '', PATHEXT: '' })), null);
});
...
test('every command spells the argv the CLI expects', async () => {
  const exec = recorder();
  const cli = claudeCli('/usr/bin/claude', runnerFor(exec));
...
test('a listing arrives as a bare array or under its own key', async () => {
  const bare = claudeCli('claude', runnerFor(recorder(listing([{ name: 'acme' }]))));
  assert.deepEqual(await bare.listMarketplaces(), [{ name: 'acme' }]);

  const wrapped = claudeCli(
    'claude',
    runnerFor(recorder(listing({ marketplaces: [{ name: 'acme' }] }))),
  );
  assert.deepEqual(await wrapped.listMarketplaces(), [{ name: 'acme' }]);

  const otherKey = claudeCli(
    'claude',
    runnerFor(recorder(listing({ plugins: [{ name: 'acme' }] }))),
  );
...
test('a CLI that cannot answer says null, never an empty answer', async () => {
  const failed = claudeCli(
    'claude',
    runnerFor(recorder({ code: 1, stderr: 'unknown option --json' })),
  );
  assert.equal(await failed.listMarketplaces(), null);
  assert.equal(await failed.listPlugins(), null);

  const garbage = claudeCli(
    'claude',
    runnerFor(recorder({ code: 0, stdout: 'Usage: claude plugin' })),
  );
...
test('junk marketplace rows are dropped, junk plugin rows make the answer unknown', async () => {
  const markets = claudeCli(
    'claude',
    runnerFor(recorder(listing([{ name: 'acme' }, 'bare', 42, null]))),
  );
  assert.deepEqual(await markets.listMarketplaces(), [{ name: 'acme' }]);

  const plugins = claudeCli(
    'claude',
    runnerFor(recorder(listing([{ id: 'xero@acme' }, { name: 'no-id' }]))),
  );
  assert.equal(await plugins.listPlugins(), null);
});

test('an empty listing is an answer, and an empty one', async () => {
  const cli = claudeCli('claude', runnerFor(recorder(listing([]))));
...
test('the boundary reports through its return value and prints nothing', async () => {
  const cli = claudeCli('claude', runnerFor(recorder({ code: 1, stderr: 'boom' })));
