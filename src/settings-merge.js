...
const { ensureDir, timestamp, stripBom } = require('./util');
...
const toKey = (dir) => dir.replace(/\\/g, '/'); // forward slashes are valid JSON on Windows
const escapeRe = (s) => s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
const eolOf = (text) => (text.includes('\r\n') ? '\r\n' : '\n');

function freshDocument(key, eol = '\n') {
...
function writeKeepingBom(file, text, hadBom) {
 ...
 */
function addPluginLocation(settingsPath, dir) {
  const key = toKey(dir);
  ...
  if (new RegExp(`"${escapeRe(KEY)}"\\s*:\\s*\\{\\s*\\}`).test(raw)) {
    ...
    out = raw.replace(/^(\s*\{)/, (_m, open) => `${open}${eol}    "${KEY}": { ${entry} },`);
    action = 'inserted-key';
  }
...
function removePluginLocation(settingsPath, dir) {
...
module.exports = { KEY, toKey, freshDocument, addPluginLocation, removePluginLocation };
