...
| `harnesses` | Unchanged, except that something has to answer whether the files must be staged locally - see the open item on where that fact lives. |
...
- Somewhere has to answer "must the files be staged locally?". Phase 1 tried
  `needsSource(origin)` on every harness and backed it out, so phase 2 decides this with
  its first caller in hand - see the open item below.
- `isSameOrigin` gains a directory arm: match `source === 'directory'` and `path` against
...
| Command     | Change                                                                                                                                                                                                                                                                                                                                                               |
| ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `install`   | Parses the spec at `resolve`. Confirms a non-marketplace source once, before any of its files are fetched or copied, through the existing `Ask` seam - `--yes` skips it, and a non-interactive shell prints the resolved source and proceeds, the way `nobodyToAsk` already does. A cancel is `ActionResult.cancelled`, so exit 130 and the session still cleans up. |
...
| `types/harness.ts`                                                | edit   | `HarnessContext` carries a `MarketplaceOrigin` as `origin`. Phase 1.              |
...
| 2   | Install from a local directory             | `feat:`     | The parser's local arm, `local-plugin.ts`, `local-marketplace.ts`, the Claude directory arm, the trust prompt, the manifest key, `installed` rendering, `source_kind`. The generated registry shipped without the `owner` Claude's schema requires, so this arm never worked for Claude Code on any version that enforces it (2.1.269 and 2.1.270 were both checked) - found by a colleague running it, not by CI, whose smoke job installs only into Cursor and VS Code and whose unit tests drive a fake runner that cannot refuse bytes.                                                                                                                                                                                                                                                                                                                                                                                     |
| 3   | Install from a GitHub repo or subdirectory | `feat:`     | The parser's github arm, `readPluginManifest`, the root checkout in both fetch arms, the marketplace-not-a-plugin hint. Done: 651 tests pass. Four things the plan did not have: `restoreSource` / `sourceKindOf`, because three key shapes is where two readers became one; `reportableId(learned)`, because a github source does not know its id until the manifest is read; `ManifestContext.locate` widened from "a local row" to "a row not keyed by a marketplace", without which a github row was unreachable by id; and the fetch condition becoming compound, which is what settles the `needsSource` open item below. Two defects in this arm were found later, by asking what a pasted URL does: an `@ref` was split only off the shorthand, so no URL or scp address could carry one, and a github.com view word was read as a folder - `.../blob/main/tools/foo` installed nothing and blamed a path nobody typed. |
| 4   | Re-sync path plugins on update             | `feat:`     | The `restore` branch per row and the `unavailable` arm. Held back so phases 2 and 3 ship without `update` having to know about either yet. Done: 661 tests pass. Two departures from the plan: `unavailable` covers a missing _directory_ only - a repository that cannot be read fails its row, because a 404 and an outage are not distinguishable from here - and `InstallRequest.plugin` widened to `string \| PluginSource`, so a row hands over the source it restored rather than spelling it back out as an argument for the action to re-parse.                                                                                                                                                                                                                                                                                                                                                                        |
| 5   | Document the new sources                   | `docs:`     | README examples and the `--help` block. Contributor and agent knowledge goes to `CLAUDE.md` as each phase lands, not here. Done, folded into phases 2-4 as each shipped; the final pass was the troubleshooting table, which had learned none of the new failures, and the two places that still described `update` as reporting a path plugin rather than refreshing it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
- ~~**Does `marketplace update` exit 0 for a directory marketplace?**~~ Answered against
  claude 2.1.269: **yes**, and it re-reads the directory on the way ("Validating local
  marketplace"). More usefully, the same probe answered the question underneath it - for a
  marketplace Claude does _not_ hold, `update` exits **1** with "Marketplace 'x' not
  found". That is what makes `add` and `update` failing together proof that nothing is
  registered, which is now how the harness reads the pair rather than reporting success.
- ~~**Does Claude read a directory marketplace in place, or copy it?**~~ Answered: the
  _marketplace_ is read in place (`path` and `installLocation` are both the directory we
  generated), while the _plugin_ is copied to
  `<config>/plugins/cache/<marketplace>/<id>/<version>`. So the `marketplace update` in
  the staging path is not redundant: it is what makes Claude re-read the registry before
  the install copies the new version out of it.
...
- **Should the origin hold a `RepoSlug` rather than a `string`?** (Phase 3 answered the
  ...
  is why it is not in phase 1; worth doing before a second origin arm makes it two
  refactors.
